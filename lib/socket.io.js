function PongServer(webServer){
	...
	io.sockets.on('connection', function(socket){
		...
		socket.on('joinGame', function(args){
			...
			if(inRoom.length === 1){
				...
				io.sockets.in(me.gameRoom).emit('updateScoresInfos', allRooms[me.gameRoom]);
				startGame();
		...
		socket.on('endGame', function(){
			if(!allRooms[me.gameRoom].gameInfos.ended){
				...
				++allRooms[me.gameRoom][definePaddle(me.paddle)].score;
			}
		});
		socket.on('playAgain', function(again){
			...
			if(opponent.playAgain !== null){
				if(opponent.playAgain && again){
					io.sockets.in(me.gameRoom).emit('playAgain', {
						playAgain: true,
						gameInfos: allRooms[me.gameRoom]
					});
					startGame();
				} else{
					io.sockets.in(me.gameRoom).emit('playAgain', {
	...
	function startGame(gameRoom){
		setTimeout(function(){
			io.sockets.in(gameRoom).emit('readyToPlay');
		}, 500);
	}
}
